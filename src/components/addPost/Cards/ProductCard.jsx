...
//   const [location,setLocation] = useState("")
  ...
  const [data,setData]= useState(
    ...
    async function onSaveHandler() {
      ...
      requestData.price=data.price!==price && price!==""?price:data.price;
      ...
      const response = await fetch(`${postInfo}/add-oldProducts`, {
        method: 'POST', 
        mode: 'cors', 
        cache: 'no-cache', 
    ...
    return(<ThemeProvider theme={theme}>
      { console.log(props)}
         ...
         <Grid alignSelf="center">
              <Typography variant="h4">Product Update</Typography>
              </Grid>
        ...
        <Typography>{picture!==undefined && picture!==null?picture.name:""}</Typography>
          ...
          <br/><br/>
        <Typography variant="h5" component="h2" style={{display: 'inline-block'}} color="textSecondary">price:
          </Typography> 
          {isEdit?
          <Typography value={price} variant="h5" component="h2" style={{display: 'inline-block'}}>{price}</Typography>:
          <TextField value={price} onChange={(e)=>{setprice(e.target.value)}}></TextField>
          }
          <br/><br/>
